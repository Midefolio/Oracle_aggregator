...
const isSending =(action, msg)=> {
 var btn = document.getElementById('isSending');
 var textCon = document.getElementById('sending-msg')
 if(btn){
  if(action === true){
   btn.style.display = 'block';
   if(msg){textCon.innerHTML = msg}
  }else{
...
function formatNumber(num) {
  if (num) {
    ...
    const numString = integerPart.toString().split("");
    ...
    // Join the formatted integer part and add the fractional part
    return formattedNum.join("") + "." + fractionalPart;
  }
  return null;
}
...
function preventSpaceInPassword(event) {
  const passwordInput = document.querySelectorAll(id); // Replace 'yourPasswordInputId' with the actual ID of your password input element
  if (event.code === 'Space') {
...
function hasDatePassed(dateString, timeString) {
  ...
  const timeParts = timeString.split(/(?=[ap]m)/i); // Split by 'am' or 'pm'
  ...
  const inputTime = new Date(year, month, day, hours, minutes);

  // Create a Date object for the current date and time
  const currentDate = new Date();

  // Compare the input date and time with the current date and time
  return inputTime < currentDate;
}
